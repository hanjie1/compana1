struct vtp_data_struct
{
   ...
   unsigned int slot_id_strip; // vetroc slot id
   unsigned int chan;          // vetroc channel
   unsigned int scal_cnt;      // vetroc scaler counts
	  ...
	  switch(vtp_data.sub_type){
	   ...
	   case 10:
		  if(vtp_data.new_type){
			 ...
			 if(i_print)
				printf("%8X - STRIP SCALER - HEADER - SLOT ID = %d\n",data,vtp_data.slot_id_strip);
	      }
		  else{
			 if(sub_type_10 < 129){
			    ...
			    if(i_print)
				  printf("%8X - STRIP SCALER - CHAN %d scaler counts = %d\n", data, vtp_data.chan, vtp_data.scal_cnt);
			 }	
		  }
		  sub_type_10++;
		  if(sub_type_10 > 129){
