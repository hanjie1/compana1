...
void StripPattern()
{
     ...
     for(int ii=0;ii<nentries;ii++){
	   ...
	   Double_t cav_rate = 0;
	   if(clock!=0){
		  cav_rate=CavPower*1.0/(clock*1.0/(40.0e6));
	   ...
	   for(int jj=0;jj<eplaneA_nhits;jj++){
		 int nstrip = 0;
	     ...
	     hA->Fill(nstrip);	
		 if(laser_on)hA_on->Fill(nstrip);
 	   }

	   for(int jj=0;jj<eplaneB_nhits;jj++){
		 int nstrip = 0;
	     ...
	     hB->Fill(nstrip);	
		 if(laser_on)hB_on->Fill(nstrip);
 	   }

	   for(int jj=0;jj<eplaneC_nhits;jj++){
		 int nstrip = 0;
	     ...
	     hC->Fill(nstrip);	
		 if(laser_on)hC_on->Fill(nstrip);
 	   }

	   for(int jj=0;jj<eplaneD_nhits;jj++){
		 int nstrip = 0;
